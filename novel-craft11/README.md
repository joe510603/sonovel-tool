...
### 📖 EPUB 转 Markdown
- 将 EPUB 小说转换为 Markdown 格式，在 Obsidian 中直接阅读
- 每个章节生成独立的 Markdown 文件，支持双向链接
- 自动生成书籍管理文档，包含章节目录和阅读进度
- 自动生成书库总览，统计所有已转换的书籍
- 打开章节时自动追踪阅读进度
- 支持批量转换多个 EPUB 文件

### ✏️ 交互式拆书 (v1.5 新增)
全新的作者主导深度拆书功能，边阅读边标记，结合 AI 模板分析：
...
| 书库路径 | `NovelCraft/books` | EPUB 转换后的书籍保存位置 |
...
| `NovelCraft: 打开交互式拆书` | 打开交互式拆书面板 |
| `NovelCraft: 打开素材库` | 打开跨书籍素材库 |
...
NovelCraft/
├── books/                      # EPUB 转换后的书籍
│   ├── 00-书库总览.md          # 书库统计和书籍列表
│   ├── .library-data.json      # 书库数据（隐藏文件）
│   └── 书名/
│       ├── 00-书名-管理.md     # 书籍管理文档（进度、目录）
│       ├── 01-第一章.md        # 章节文件
│       ├── 02-第二章.md
│       └── ...
├── notes/                      # 分析笔记
...
│           ├── 001-第一章.md
│           └── ...
├── marks/                      # 交互式拆书数据 (v1.5 新增)
│   └── 书名/
│       ├── marks.json          # 标记数据
│       ├── plots.json          # 故事情节分析
│       ├── characters.json     # 人物追踪数据
│       └── settings.json       # 设定信息
└── library/                    # 跨书籍素材库 (v1.5 新增)
    ├── settings/               # 设定信息汇总
    │   ├── 境界设定.md
    │   ├── 功法体系.md
    │   └── ...
