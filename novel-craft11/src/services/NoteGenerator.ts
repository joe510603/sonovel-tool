...
export class NoteGenerator {
  ...
  generatePlotNote(analysis: AnalysisResult): string {
    ...
    if (analysis.chapterStructure && analysis.chapterStructure.length > 0) {
      ...
      for (const chapter of analysis.chapterStructure) {
        // 使用 Obsidian 链接格式引用章节 (Requirements: 6.1, 6.2)
        const chapterLink = this.generateChapterLink(
          chapter.index,
          chapter.title,
          analysis.bookInfo.title,
          `第${chapter.index + 1}章: ${chapter.title}`
        );
        lines.push(`### ${chapterLink}`);
        lines.push('');
    ...
    if (analysis.emotionCurve && analysis.emotionCurve.length > 0) {
      lines.push('## 情绪曲线');
      lines.push('');
      lines.push(this.generateEmotionCurveTable(analysis.emotionCurve, analysis.bookInfo.title));
      lines.push('');
      ...
      if (climaxPoints.length > 0) {
        ...
        for (const point of climaxPoints) {
          // 使用章节链接 (Requirements: 6.1, 6.2)
          const chapterLink = this.generateChapterLink(
            point.chapter - 1, // chapter 是 1-based
            `chapter-${point.chapter}`,
            analysis.bookInfo.title,
            `第${point.chapter}章`
          );
          lines.push(`- **${chapterLink}** (强度: ${point.intensity}/10): ${point.description}`);
        }
        lines.push('');
      }
    }
    ...
    if (analysis.foreshadowing && analysis.foreshadowing.length > 0) {
      lines.push('## 伏笔设计');
      lines.push('');
      lines.push(this.generateForeshadowingSection(analysis.foreshadowing, analysis.bookInfo.title));
    }
  ...
  /**
   * 生成情绪曲线表格
   * Requirements: 6.1, 6.2 - 使用 Obsidian 链接格式引用章节
   */
  private generateEmotionCurveTable(emotionCurve: EmotionPoint[], bookTitle?: string): string {
    const lines: string[] = [];
    ...
    for (const point of emotionCurve) {
      ...
      // 使用章节链接 (Requirements: 6.1, 6.2)
      const chapterDisplay = bookTitle
        ? this.generateChapterLink(
            point.chapter - 1, // chapter 是 1-based
            `chapter-${point.chapter}`,
            bookTitle,
            `第${point.chapter}章`
        ...
        : `第${point.chapter}章`;
      lines.push(`| ${chapterDisplay} | ${intensityBar} ${point.intensity}/10 | ${point.description} |`);
    }
   ...
   * Requirements: 6.1, 6.2 - 使用 Obsidian 链接格式引用章节
   */
  private generateForeshadowingSection(foreshadowing: Foreshadowing[], bookTitle?: string): string {
    const lines: string[] = [];
    ...
    // 生成章节链接的辅助函数
    const getChapterLink = (chapter: number): string => {
      if (bookTitle) {
        return this.generateChapterLink(
          chapter - 1, // chapter 是 1-based
          `chapter-${chapter}`,
          bookTitle,
          `第${chapter}章`
        );
    ...
    if (resolved.length > 0) {
      ...
      for (const f of resolved) {
        const setupLink = getChapterLink(f.setupChapter);
        const payoffLink = f.payoffChapter ? getChapterLink(f.payoffChapter) : '?';
        lines.push(`- **${setupLink} → ${payoffLink}**: ${f.description}`);
      }
      lines.push('');
    }
    ...
    if (planted.length > 0) {
      ...
      for (const f of planted) {
        const setupLink = getChapterLink(f.setupChapter);
        lines.push(`- **${setupLink}埋下**: ${f.description}`);
      }
      lines.push('');
    }
    ...
    if (abandoned.length > 0) {
      ...
      for (const f of abandoned) {
        const setupLink = getChapterLink(f.setupChapter);
        lines.push(`- **${setupLink}**: ${f.description}`);
      }
      lines.push('');
    }
    ...
    if (deepModeRanges.length > 0 && analysis.chapterDetails && analysis.chapterDetails.length > 0) {
      ...
      for (const detail of deepChapterDetails) {
        ...
        const chapterLink = this.generateChapterLink(
          ...
          detail.title,
          analysis.bookInfo.title,
          `第${detail.index + 1}章: ${detail.title}`
        );
        lines.push(`### ${chapterLink}`);
        lines.push('');
        lines.push(detail.analysis);
  ...
  /**
   * 生成章节链接
   * 使用 Obsidian 链接格式 [[章节文件|显示文本]]
   * Requirements: 6.1, 6.2
   * 
   * @param chapterIndex 章节索引 (0-based)
   * @param chapterTitle 章节标题
   * @param bookTitle 书籍标题
   * @param displayText 可选的显示文本，默认为 "第X章: 标题"
   * @returns Obsidian 链接格式的字符串，如果章节不存在则返回纯文本
   */
  generateChapterLink(
    chapterIndex: number,
    chapterTitle: string,
    ...
    const chapterFilename = `${String(chapterNum).padStart(2, '0')}-${sanitizedChapterTitle}`;
    
    // 检查章节文件是否存在
    const chapterExists = this.config.checkChapterExists
      ? this.config.checkChapterExists(chapterIndex, chapterTitle)
      : true; // 默认假设存在
    
    const display = displayText || `第${chapterNum}章: ${chapterTitle}`;
    
    // 优先使用 bookFolderPath，其次使用 convertedBooksPath + bookTitle
    let targetPath: string | undefined;
    if (this.config.bookFolderPath) {
      targetPath = this.config.bookFolderPath;
    } else if (this.config.convertedBooksPath) {
      targetPath = `${this.config.convertedBooksPath}/${sanitizedBookTitle}`;
    }
    
    if (chapterExists && targetPath) {
      // 使用相对路径生成链接
      return `[[${targetPath}/${chapterFilename}|${display}]]`;
    } else if (chapterExists) {
      // 没有书籍路径时使用简单链接
      return `[[${chapterFilename}|${display}]]`;
    }
