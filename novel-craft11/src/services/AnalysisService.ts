 ...
 * 支持快速、标准、深度三种分析模式。
 * 
 * 重构说明 (Requirements: 11.2, 11.3):
 * - 分析结果自动同步到数据库
 * - 调用 DataSyncService.syncAnalysisResult()
 */
...
import { DataSyncService, SyncResult } from './DataSyncService';
...
export class AnalysisService {
  ...
  /** 转换后书籍的根目录（如 NovelCraft/books），用于生成章节链接 */
  private convertedBooksPath: string | null = null;
  /** 检查章节文件是否存在的函数 */
  private checkChapterExists: ((chapterIndex: number, chapterTitle: string) => boolean) | null = null;
  
  /** 数据同步服务 (Requirements: 11.2) */
  private dataSyncService: DataSyncService | null = null;
  /** 书籍数据库服务 */
  private bookDatabaseService: BookDatabaseService | null = null;
  /** 书籍路径缓存: bookId/bookTitle -> bookPath */
  ...
  /**
   * 设置数据同步服务
   * @param dataSyncService 数据同步服务实例
   * 
   * Requirements: 11.2
   */
  setDataSyncService(dataSyncService: DataSyncService): void {
    this.dataSyncService = dataSyncService;
  }

  /**
   * 设置书籍数据库服务
   * @param bookDatabaseService 书籍数据库服务实例
   */
  ...
  /**
   * 注册书籍路径映射
   * @param bookKey - 书籍标识（可以是 bookId 或 bookTitle）
   * @param bookPath - 书籍文件夹路径
   */
  registerBookPath(bookKey: string, bookPath: string): void {
    this.bookPathCache.set(bookKey, bookPath);
  }
   ...
   */
  private getBookPath(bookKey: string): string | null {
    return this.bookPathCache.get(bookKey) || null;
  }

  /**
   * 设置转换后书籍的路径配置
   * 用于在分析笔记中生成正确的章节链接
   * @param convertedBooksPath 转换后书籍的根目录
   * @param checkChapterExists 检查章节文件是否存在的函数
   */
  setConvertedBooksPath(
    convertedBooksPath: string,
    checkChapterExists?: (chapterIndex: number, chapterTitle: string) => boolean
  ): void {
    this.convertedBooksPath = convertedBooksPath;
    this.checkChapterExists = checkChapterExists || null;
  }
   ...
   * 更新进度
   * @param stage 阶段名称
   * @param progress 进度百分比，-1 表示不更新进度条
   * @param message 进度消息
   */
  private updateProgress(stage: string, progress: number, message: string): void {
    if (this.onProgress) {
      // 如果 progress 为 -1，则保持当前进度不变
      if (progress < 0) {
        this.onProgress({ stage, progress: -1, message });
      } else {
   ...
   * @param book 已解析的书籍
   * @param config 分析配置
   * @param stageId 阶段 ID (synopsis, characters, techniques, etc.)
   * @param onProgress 进度回调
   * @returns 该阶段的分析结果
   */
  async retrySingleStage(
    book: ParsedBook,
    ...
    switch (stageId) {
      case 'synopsis':
        stageResult = await this.analyzeSynopsis(book, chunks, config);
        break;
      case 'characters':
        stageResult = await this.analyzeCharacters(book, chunks, config);
        break;
      case 'techniques':
        stageResult = await this.analyzeTechniques(book, chunks, config);
        break;
      case 'takeaways':
        stageResult = await this.analyzeTakeaways(book, chunks, config, result);
        break;
      case 'emotionCurve':
        stageResult = await this.analyzeEmotionCurve(book, chunks, config);
        break;
      case 'chapterStructure':
        stageResult = await this.analyzeChapterStructure(book, chunks, config);
        break;
      case 'foreshadowing':
        stageResult = await this.analyzeForeshadowing(book, chunks, config);
        break;
      case 'chapterDetail':
        stageResult = await this.analyzeChapterDetails(book, config);
        break;
      case 'writingReview':
        stageResult = await this.analyzeWritingReview(book, config, result);
        break;
      default:
        throw new Error(`未知的分析阶段: ${stageId}`);
    }
  ...
  async analyze(
    ...
    // 同步分析结果到数据库 (Requirements: 11.2)
    await this.syncResultToDatabase(book.metadata.title, result);
  ...
  async analyzeBatched(
    ...
    if (createFile && outputPath) {
      ...
      // 构建 NoteGenerator 配置，包含章节链接所需的路径信息
      ...
      if (this.convertedBooksPath) {
        generatorConfig.convertedBooksPath = this.convertedBooksPath;
      }
      if (this.checkChapterExists) {
        generatorConfig.checkChapterExists = this.checkChapterExists;
      }
      
      const generator = new NoteGenerator(generatorConfig);
  ...
  private async analyzeSynopsis(
    ...
    // 注意：不更新进度百分比，由外层循环控制
    this.updateProgress('故事梗概', -1, '故事梗概: 正在采样关键章节内容...');
  ...
  private async analyzeTakeaways(
    ...
    this.updateProgress('可借鉴清单', -1, '可借鉴清单: 正在整合已有分析结果...');
...
`;
    ...
    this.updateProgress('可借鉴清单', -1, '可借鉴清单: 正在等待 AI 响应... ⏳');
    
    const response = await this.sendLLMRequest(prompt, context, config.customPrompts);
    
    this.updateProgress('可借鉴清单', -1, '可借鉴清单: AI 响应完成，正在解析...');
    ...
    this.updateProgress('可借鉴清单', -1, `可借鉴清单: 完成 ✓ 已生成 ${normalizedTakeaways.length} 条借鉴要点`);
  ...
  private async analyzeEmotionCurve(
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // 确保 emotionCurve 是数组
      if (Array.isArray(parsed.emotionCurve)) {
  ...
  private async analyzeChapterStructure(
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // 确保 chapterStructure 是数组
      if (Array.isArray(parsed.chapterStructure)) {
  ...
  private async analyzeForeshadowing(
    ...
    this.updateProgress('伏笔分析', -1, '伏笔分析: 正在采样全书关键章节...');
    ...
    this.updateProgress('伏笔分析', -1, '伏笔分析: 正在等待 AI 响应... ⏳');
    
    const response = await this.sendLLMRequest(prompt, sampledContent, config.customPrompts);
    
    this.updateProgress('伏笔分析', -1, '伏笔分析: AI 响应完成，正在解析...');
    ...
    this.updateProgress('伏笔分析', -1, `伏笔分析: 完成 ✓ 已识别 ${parsed.foreshadowing.length} 处伏笔`);
  ...
  private async analyzeWritingReview(
    ...
    this.updateProgress('写作复盘', -1, '写作复盘: 正在整合全书分析数据...');
...
`;
    ...
    this.updateProgress('写作复盘', -1, '写作复盘: 正在等待 AI 响应... ⏳');
    
    const response = await this.sendLLMRequest(prompt, context, config.customPrompts);
    
    this.updateProgress('写作复盘', -1, '写作复盘: 完成 ✓ 复盘报告生成完毕');
   ...
   */
  private async syncResultToDatabase(
    bookTitle: string,
    result: AnalysisResult
  ): Promise<SyncResult | null> {
    // 如果没有配置 DataSyncService，跳过同步
    if (!this.dataSyncService) {
      return null;
    }

    // 获取书籍路径
    const bookPath = this.getBookPath(bookTitle);
    if (!bookPath) {
      console.warn(`无法获取书籍路径，跳过数据库同步: ${bookTitle}`);
      return null;
    }

    try {
      // 调用 DataSyncService 同步分析结果
      const syncResult = await this.dataSyncService.syncAnalysisResult(bookPath, result);
      
      if (!syncResult.success) {
        console.error('同步分析结果到数据库失败:', syncResult.errors);
      } else {
        console.log(`分析结果已同步到数据库，共同步 ${syncResult.syncedCount} 条记录`);
      }
      
      return syncResult;
    } catch (error) {
      console.error('同步分析结果到数据库时发生错误:', error);
      return {
        success: false,
        syncedCount: 0,
        conflicts: [],
