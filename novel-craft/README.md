...
- **数据库存储**：使用 IndexedDB 本地数据库，支持导入数据的持久化存储
- **文件组织**：自动创建书籍文件夹，生成完整的元数据和目录文件

### 📖 故事单元管理
- **故事单元划分**：将小说按情节单元划分，便于结构化分析
- **轨道分组**：支持主线、支线、回忆等多轨道管理
- **编辑器工具栏**：在章节编辑器中快速标记和创建故事单元
- **侧边栏视图**：统一管理所有故事单元，支持内联编辑

### 🤖 AI 故事拆解
- **七步故事法**：基于网文爽点循环结构的专业拆书模板
  - 主角优势→反派+信息差→初次摩擦→负面预期→高潮反杀→震惊反应→收获+升级
- **故事梗概**：自动生成50-100字的情节概括
- **情绪折线**：分析读者情绪起伏变化
- **人物关系**：自动识别友方/中立/敌方人物关系
- **精简表格**：分析结果以关键词形式展示，支持手动编辑
- **完整报告**：自动生成详细的MD分析文档
- **实时显示**：分析过程中实时更新表格内容
...
- **方式一**：直接将文档文件拖入 Vault（支持 epub/txt/docx/pdf）
  - EPUB 文件自动解析章节结构，保持原书格式
  - 自动生成书籍文件夹和元数据文件
  - 支持导入进度显示和错误提示
- **方式二**：使用主面板搜索下载（需配置 SoNovel 服务）
- **数据持久化**：所有导入数据自动保存到本地数据库，支持后续查询和管理
...
项目包含完整的测试套件：
- **单元测试**：验证具体功能和边界情况
- **属性测试**：使用 fast-check 进行属性验证，确保核心功能的正确性
- **集成测试**：验证各组件间的协作
